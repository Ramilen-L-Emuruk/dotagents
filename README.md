...
今回取り込んだ共通規約は、合意済みの範囲を勝手に縮小しないこと、機能削除前に実利用を調査すること、ビルドだけでなく適切な動作確認を行うこと、レビューを無限に繰り返さず残存リスクを明示すること。プロジェクト固有のビルド・デプロイ・API・UI仕様は、それぞれのプロジェクトの `AGENTS.md` に残す。
