...
2. Create a clearly named branch and worktree with Git. Keep project-local worktrees under `.codex/worktrees/` only when the target repository's `.gitignore` includes that path; otherwise use the repository's documented location.
